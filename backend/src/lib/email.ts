...
export async function sendVerificationEmail(options: SendVerificationEmailOptions): Promise<EmailResult> {
  try {
    ...
    const smtpPass = process.env.BREVO_SMTP_PASSWORD || process.env.BREVO_SMTP_KEY;
    const apiKey = process.env.BREVO_API_KEY;

    const senderEmail = process.env.EMAIL_FROM || "noreply@nssdirectstay.gh";
    ...
    if (smtpUser && smtpPass) {
      try {
        ...
        try {
          ...
          nodemailerModule = req("nodemailer");
        } catch {
          // Module not loaded directly
        }
        ...
        if (nodemailerModule) {
          ...
          return {
            success: true,
            message: `Verification email sent successfully to ${toEmail} via Brevo SMTP.`,
            provider: "BREVO_SMTP",
          };
        }
      } catch (smtpErr: any) {
        console.error("Brevo SMTP Delivery Error:", smtpErr?.message || smtpErr);
        // Fallthrough to API or demo fallback
      }
    }

    // 2. Try Brevo REST API v3
    if (apiKey) {
      try {
    ...
    // 3. Fallback Demo Logger (When no credentials configured)
    console.log("=================================================");
    ...
    // Catch-all safety guard to ensure process NEVER crashes
    console.error("Unhandled Email Service Exception:", err?.message || err);
    return {
      success: false,
